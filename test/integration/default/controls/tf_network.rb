control "Terraform Network" do
  ...
  describe "network zone" do
    it { expect(input("network").fetch(:subnet).fetch(:network_zone)).to eq input("hetzner_network_zone") }
  end

  describe "CIDR" do
    it {
      expect(input("network").fetch(:subnet).fetch(:ip_range)).to eq input("hetzner_network_ip_range").gsub(".0.0/16",
                                                                                                            ".1.0/24")
    }
  end
  ...
  describe "gateway IP" do
    it {
      expect(input("network").fetch(:subnet).fetch(:gateway)).to eq input("hetzner_network_ip_range").gsub(".0.0/16",
                                                                                                           ".0.1")
    }
  end
end
